...
export function registerIpcHandlers(storageManager: StorageManager): void {
  ...
  ipcMain.handle('storage:save-custom-model', async (_event, newModel: CustomModelFileEntry & { apiKey?: string }) => {
    ...
    try {
      ...
      // ponytail: key on name+provider so same model id on different providers coexists instead of overwriting.
      const existingIdx = models.findIndex((m) => m.name === newModel.name && (m.provider || '') === (newModel.provider || ''));
      ...
      await fs.writeFile(filePath, JSON.stringify({ models }, null, 2), { encoding: 'utf-8', mode: 0o600 });
      try {
  ...
  ipcMain.handle('storage:delete-custom-model', async (_event, target: string | { name: string; provider?: string }) => {
    const geminiDir = path.join(app.getPath('home'), '.gemini', 'antigravity');
    const filePath = path.join(geminiDir, 'custom_models.json');
    try {
      let models: CustomModelFileEntry[] = [];
      ...
      // ponytail: delete by name+provider when provider known, else legacy name-only match
      const name = typeof target === 'string' ? target : target.name;
      const provider = typeof target === 'string' ? undefined : target.provider;
      models = models.filter((m) => (provider ? !(m.name === name && m.provider === provider) : m.name !== name));
