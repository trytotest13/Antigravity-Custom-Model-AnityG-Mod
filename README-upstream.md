...
- **`install.bat`** - builds the mod and patches Antigravity (backs up `app.asar` and `language_server.exe` first, then restarts the IDE). Re-run it after every Antigravity update.
- **`uninstall.bat`** - closes Antigravity and restores the original `app.asar` / `app.asar.unpacked` / `language_server.exe` from the backups; if a backup is missing it reverses the URL patch in place. Backups are kept so `install.bat` can be re-run later.
